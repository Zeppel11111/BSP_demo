...
/* =====================================================================
 * 模块开关：CFG_ENABLE_FONT8X16 与 OLED 共享（app_config.h 中定义为
 * CFG_ENABLE_OLED 的别名），= 0 时整个模块不编译，
 * 调用方（OLED 驱动）靠 font8x16.h 里的裁剪桩拿到 NULL。
 * ===================================================================== */
#if CFG_ENABLE_FONT8X16
/* ---- 8x16 ASCII 字库：95 字符（0x20~0x7E），每字符 16 字节，每字节一行 8 像素 ----
 * 来源：Windows 新宋体 16px 内嵌点阵，程序化生成，见工程注释 */
static const uint8_t font8x16_table[FONT8X16_COUNT][16] = {
    /* 0x20 ' ' */
...
{
    if (ch < FONT8X16_FIRST || ch > FONT8X16_LAST)
    {
        ch = FONT8X16_FIRST;   /* 越界统一落回空格，保证指针合法 */
    }
    return &font8x16_table[(uint8_t)ch - FONT8X16_FIRST][0];
}
